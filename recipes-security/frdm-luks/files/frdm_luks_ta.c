                                ...
                                TEE_Param params[TEE_NUM_PARAMS])
{
    ...
    /* Report the required size so the caller can retry. */
    ...
    /*
     * The System PTA requires the derived-key output in secure memory, so derive
     * into a TA-owned buffer and copy the result to the normal-world caller.
     */
    ...
    /*
     * Reached on the success path and on every failure that can leave data in
     * secret. memzero_explicit is used instead of memset because it survives
...
/*
 * There is no unlock command, by design. The lock therefore holds until
 * OP-TEE restarts.
 */
...
/*
 * OP-TEE requires these entry points even when they do nothing. This TA
 * holds no per-session state, so nothing has to be set up or torn down.
 */
